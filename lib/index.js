...
function Uber(data) {
  ...
  if (typeof data === 'string') {
    ...
    if (typeof data.use_sandbox === 'undefined') {
      use_sandbox = false;
    } else {
...
//    callback = (typeof callback === 'undefined') ? function(e,b){} : callback;
    return get(this.getAuthToken(), buildUrl()+"/products?latitude="+latitude+"&longitude="+longitude, callback);
  }
  ...
  function getPriceEstimate(start_latitude, start_longitude, end_latitude, end_longitude, callback) {
    var u = buildUrl() + "/estimates/price?";
...
//    callback = (typeof callback === 'undefined') ? function(e,b){} : callback;
    var u = buildUrl()+"/estimates/time?start_latitude="+start_latitude+"&start_longitude="+start_longitude;
...
//    callback = (typeof callback === 'undefined') ? function(e,b){} : callback;
    var u = buildUrl()+"/promotions?start_latitude="+start_latitude+"&start_longitude="+start_longitude;
    u += "&end_latitude="+end_latitude+"&end_longitude="+end_longitude;
    return get(this.getAuthToken(), u, callback);
  }
  ...
  function buildUrl() {
    var str = url+api_version;
    if (use_sandbox) {
      str = sandbox_url + api_version + "/sandbox/";
    }
    return str;
  }
